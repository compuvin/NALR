...
sTypeChange = Request.Form("TypeChange")
sPermission = Request.Form("Permission")
sAcknowleged = Request.Form("Acknowleged")
sStartDate = Request.Form("StartDate")
sEndDate = Request.Form("EndDate")

URLRed = "index.asp?filter=Search"
...
if not sTypeChange = "Any" then URLRed = URLRed & "&typechange=" & sTypeChange
if not sPermission = "Any" then URLRed = URLRed & "&permission=" & sPermission
if not sAcknowleged = "Any" then URLRed = URLRed & "&acknowleged=" & sAcknowleged
if not sStartDate = "" then URLRed = URLRed & "&StartDate=" & format(sStartDate, "YYYYMMDD")
if not sEndDate = "" then URLRed = URLRed & "&EndDate=" & format(sEndDate, "YYYYMMDD")

if URLRed = "index.asp?filter=Search" then URLRed = "index.asp"
